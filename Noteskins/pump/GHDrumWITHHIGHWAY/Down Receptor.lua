...
if sButton == "Center" or (rButton == 1 and string.find(GAMESTATE:GetCurrentStyle(pn):GetStepsType() ,"Double") )then
	t = Def.ActorFrame {
		Def.Sprite {
			Texture=NOTESKIN:GetPath("","Highway "..Colour[sButton]);
			OnCommand=function(self)
			local playerscoll = SCREENMAN:GetTopScreen():GetChild("PlayerP1"):GetChild("CMod")
			ms.ok(playerscroll)
			...
			InitCommand=function(self)
				self:diffusealpha(0):zoomto(64,4000):fadetop(0):texcoordvelocity(0,5)
			end;
		};
	};
else
	t = Def.ActorFrame {
		...
		OnCommand=function(self)
			...
			local p1 = SCREENMAN:GetTopScreen():GetChild("PlayerP1")
        		...
        		GAMESTATE:ApplyStageModifiers("PlayerP1","260% Hallway")
        		GAMESTATE:ApplyStageModifiers("PlayerP1","-35% Mini")
    		end;
		...
		Def.Sprite {
			Texture=NOTESKIN:GetPath("","Highway "..Colour[sButton]);
			InitCommand=function(self)
				self:texturewrapping(false):zoomto(64,3200):fadetop(.05):texcoordvelocity(0,-0.337):diffusealpha(1)
			end,
			OnCommand=function(self)
				if STATSMAN:GetCurStageStats():GetPlayerStageStats("PlayerP1"):GetFailed() == true then
