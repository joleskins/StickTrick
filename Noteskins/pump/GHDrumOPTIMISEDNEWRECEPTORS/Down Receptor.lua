...
if sButton == "Center" or (rButton == 1 and string.find(GAMESTATE:GetCurrentStyle(pn):GetStepsType() ,"Double") )then
	...
	t = Def.ActorFrame {
		...
		Def.ActorFrame {
			OnCommand=function(self)
				self:rotationx(90):rotationz(CustomRotation[sButton][1]):z(Position[sButton][2]):zoom(.51)
			end;
			Def.Sprite {
				Texture=NOTESKIN:GetPath("","Tex/_ReceptorRING "..Colour[sButton]);
				InitCommand=function(self)
					self:diffusealpha(1)
				end;
				Oncommand=function(self)
			...
			Def.Sprite {
				Texture=NOTESKIN:GetPath("","Tex/_ReceptorTOP "..Colour[sButton]);
				Initcommand=function(self)
					self:diffusealpha(1)
				end;
				Oncommand=function(self)
				...
				JudgmentMessageCommand=function(self, params)
					...
					if params.FirstTrack ~= ColumnNumbers[sButton]-1 then
                		return
            		end
            		if not params.TapNoteScore then
           			return
            		end;

            		local a = params.TapNoteScore:gsub("TapNoteScore_W", "")
            		if tonumber(a) == nil then
            			return
            		elseif tonumber(a) < 5 then
            			self:finishtweening():addy(-35):linear(.1):addy(35)
					end;
				end;
			};
			Def.Sprite {
				Texture=NOTESKIN:GetPath("","Tex/_ReceptorHL "..Colour[sButton]);
				InitCommand=function(self)
				...
				PressCommand=function(self)
					self:finishtweening():diffusealpha(1):linear(.05):diffusealpha(0)
				end;
				JudgmentMessageCommand=function(self, params)
	...
	for i=1,2 do
		for i2=1,15 do
			...
			Def.Sprite {
				...
				Delay0005=.016;
				InitCommand=function(self)
					self:diffuse(color("#eb6523")):diffusealpha(0):zoom(1):zoomy(1.3):draworder(99999999999999999999999999999999)
				end;
				OnCommand=function(self)
				...
				MoveCommand=function(self)
					if play == i2 then
						self:stoptweening():setstate(0):diffusealpha(0):blend('BlendMode_Add'):linear(.095):linear(.001):diffusealpha(0)
					end;
				end;
				};
			};
